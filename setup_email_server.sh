...
read -p "Digite o domínio principal do seu servidor de e-mail (ex: suadominio.com): " dominio
echo "$dominio" > /etc/mailname
export MAIL_SUBDOM=mail

echo "✅ Iniciando instalação e configuração..."

# Etapa 1 - Pacotes básicos (corrigido para Debian)
apt update
apt install -y nano curl wget gnupg2 software-properties-common certbot python3-certbot

# Etapa 2 - Executa a configuração completa com base no script funcional
echo "🔧 Executando script de instalação de servidor de e-mail completo..."

# Início do código do emailwiz_pt.sh
#!/bin/sh
...
# Comentado para utilizar somente IPv4:
...
# Forçar o uso somente de IPv4 no Postfix:
postconf -e "inet_protocols = ipv4"

# Adicionando configurações ao Postfix
